...
3. **Rotation FIFO sur le reste (max 5 lignes supplémentaires par run)** : parmi les positions non couvertes par le croisement ci-dessus, sélectionne au maximum 5 lignes en priorisant celles dont `derniere_verification` est la plus ancienne (ou `null` en premier). Pour chacune, une recherche WebSearch dédiée prix + actualité récente (même format que Phase 3). Objectif : sur quelques sessions, l'ensemble du portefeuille personnel finit par être revu, comme pour la Phase 6.
