...
`portefeuille-utilisateur.json` est **distinct** de `historique-picks.json` : il contient les positions réelles de l'utilisateur sur Trade Republic (PEA + Compte-Titres), déclarées manuellement, pas les picks générés par `/bourse`. Certaines lignes peuvent coïncider avec d'anciens picks du système (repérable via le champ `note`) — dans ce cas, croise aussi avec le statut de l'entrée correspondante dans `historique-picks.json`.

1. **Si `portefeuille-utilisateur.json` n'existe pas à la racine du projet, saute cette phase entièrement** — ne rien inventer, ne rien demander à l'utilisateur de façon insistante.
...
8. **Mets à jour `portefeuille-utilisateur.json`** après chaque ligne traitée : `derniere_verification` (date du jour), `derniere_recommandation` (le verdict + une phrase de justification).
...
- En fin de session, committe et pousse les changements (`index.html`, `historique-picks.json`, et `portefeuille-utilisateur.json` s'il a été modifié) — voir `run-bourse.ps1` pour l'automatisation quotidienne
