...
- Avant de retenir un candidat, lis `data-privees/historique-picks.json` et vérifie s'il existe déjà une entrée `"statut": "ouvert"` sur le même ticker.
...
- Avant de finaliser la sélection, compte le nombre d'entrées `"statut": "ouvert"` dans `data-privees/historique-picks.json` (après application du contrôle de doublon ci-dessus).
...
- **Positions actives** : [nombre d'entrées `"statut": "ouvert"` dans `data-privees/historique-picks.json` après cette session] / plafond de 20 (voir Phase 2, "Plafond de positions actives"). Si le plafond est atteint ou dépassé, le mentionner explicitement.
...
- [ ] Le contrôle de doublon (Phase 2) a été effectué — aucun ticker n'a deux entrées `"ouvert"` simultanées dans `data-privees/historique-picks.json`
...
1. Lis le fichier `data-privees/historique-picks.json` (sous-dossier `data-privees/`, dépôt privé séparé — voir note en fin de document) — créer `{"picks":[]}` s'il n'existe pas encore.
...
## PHASE 7 — 📋 Portefeuille personnel de l'utilisateur (OBLIGATOIRE si le fichier existe)

**Objectif : croiser les positions réellement détenues par l'utilisateur (pas les picks générés par ce système) avec le scan du jour, pour produire une recommandation actionnable par ligne.**

`data-privees/portefeuille-utilisateur.json` est **distinct** de `data-privees/historique-picks.json` : il contient les positions réelles de l'utilisateur sur Trade Republic (PEA + Compte-Titres), déclarées manuellement, pas les picks générés par `/bourse`. Certaines lignes peuvent coïncider avec d'anciens picks du système (repérable via le champ `note`) — dans ce cas, croise aussi avec le statut de l'entrée correspondante dans `data-privees/historique-picks.json`.

1. **Si `data-privees/portefeuille-utilisateur.json` n'existe pas, saute cette phase entièrement** — ne rien inventer, ne rien demander à l'utilisateur de façon insistante.
2. **Croisement gratuit avec la Phase 1 (obligatoire, sans recherche supplémentaire)** : pour chaque ticker du portefeuille personnel, vérifie s'il apparaît dans les résultats déjà obtenus en Phase 1 (gagnants, upgrades/downgrades, profit warnings, résultats trimestriels, secteurs chauds). Si oui, cette ligne est automatiquement retenue pour une recommandation complète ce run-ci — pas besoin de justifier davantage sa sélection.
3. **Couverture complète à chaque session (pas de plafond)** : toutes les positions non couvertes par le croisement gratuit ci-dessus — donc l'ensemble des lignes éligibles (`type` confirmé, hors ETF) — reçoivent chacune une recherche WebSearch dédiée prix + actualité récente (même format que Phase 3), à chaque run. Objectif : le portefeuille personnel entier est à jour après chaque session, pas seulement après plusieurs jours de rotation. Demandé explicitement par l'utilisateur le 10 juillet 2026, en connaissance du compromis (plus de recherches web par session, sessions plus longues).
4. **Exclus les lignes `"type": "ETF"` de la recommandation catalyseur-par-catalyseur** — un ETF indicé n'a pas de thèse individuelle à valider/invalider au jour le jour ; mentionne-le seulement si son `derniere_verification` n'a jamais été fait, avec une ligne factuelle de prix, sans verdict Renforcer/Alléger/Vendre.
5. **Exclus toute ligne dont `"type"` n'est pas confirmé** (ex: `"à confirmer"`) de tout calcul de prix — rappelle simplement que l'utilisateur doit clarifier la nature exacte du produit avant tout suivi, sans deviner.
6. **Pour chaque ligne retenue (étapes 2 ou 3), applique un verdict parmi 4, avec justification factuelle et sourcée (RÈGLES ABSOLUES #1/#2/#3 s'appliquent intégralement — jamais de prix inventé, toujours daté, cross-validé)** :
   - 🟢 **Renforcer** : catalyseur positif clair et confirmé par au moins 2 sources **et** la ligne n'est pas déjà en surpondération (vérifie le poids de la ligne et de son secteur GICS dans le portefeuille total si connu — ne recommande jamais de renforcer une ligne/un secteur déjà proche ou au-delà des seuils de la note de gestion du risque de la Phase 4).
   - 🔵 **Conserver** : rien de notable trouvé, ou signaux mixtes sans direction claire. C'est le verdict par défaut, pas un échec de la recherche.
   - 🟡 **Alléger** : signal négatif réel mais pas définitif (une dégradation isolée, une détérioration partielle de la thèse) — réduire la position sans sortir totalement.
   - 🔴 **Vendre** : thèse fondamentale cassée (perte structurelle documentée, guidance revue nettement à la baisse, dégradations convergentes de plusieurs sources, "value trap" caractérisé) — pas une simple baisse de cours ponctuelle sans cause identifiée.
7. **Recherche de confirmation obligatoire en cas d'ambiguïté, sur les 16 lignes sans exception — le volume ne dispense jamais de cette étape.** Si une première recherche renvoie un prix douteux (horodatage flou, chiffre isolé, source unique) ou un signal contradictoire, relance systématiquement une **deuxième recherche dédiée** (nom complet + ticker + "cours en direct"/"live price" + heure) avant de conclure quoi que ce soit — exactement comme pour AeroVironment (Phase 6, clôture stop_declenche). Seulement si cette deuxième recherche ne résout pas l'ambiguïté, verdict 🔵 Conserver avec mention explicite de l'incertitude (jamais un verdict tranché sur une seule source, RÈGLE ABSOLUE #3). Ceci vaut autant pour la 16ᵉ ligne traitée que pour la 1ʳᵉ — ne jamais relâcher cette étape parce que la session est déjà longue.
...
Ajoute une section **📋 Ton portefeuille personnel** après le "Bilan des picks précédents", listant uniquement les lignes traitées ce run-ci (croisement + rotation), avec ticker, verdict, justification en une ou deux phrases, et le nombre de lignes encore en attente de rotation.
...
- En fin de session, committe et pousse les changements dans **les deux dépôts séparés, sans exception** :
  - Dans `Projects\bourse` (dépôt public `bourse-dashboard`, sert GitHub Pages) : uniquement `index.html`. **Ne jamais** committer `data-privees/` depuis ce dépôt — il est gitignoré précisément parce que ce dépôt est public.
  - Dans `Projects\bourse\data-privees` (dépôt privé séparé `bourse-donnees-privees`, sous-dossier avec son propre `.git`) : `historique-picks.json` et `portefeuille-utilisateur.json`.
  - Voir `run-bourse.ps1` pour l'automatisation quotidienne (gère déjà les deux commits/push séparément).

## ⚠️ Séparation public / privé — pourquoi c'est important

Ce dépôt (`bourse-dashboard`) est **public** pour que GitHub Pages fonctionne. `historique-picks.json` et `portefeuille-utilisateur.json` contiennent des données sensibles (positions réelles de l'utilisateur, prix d'acquisition) et vivent **exclusivement** dans le dépôt privé séparé `bourse-donnees-privees`, via le sous-dossier `data-privees/` (son propre `.git`, gitignoré par le dépôt public). Ne jamais committer ces deux fichiers dans le dépôt public, même par accident — l'historique en a déjà été purgé une fois (10 juillet 2026) suite à un changement de visibilité qui aurait autrement exposé des données financières personnelles.
